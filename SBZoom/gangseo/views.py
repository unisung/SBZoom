...
# /SBZoom/gangse0/
def index(request):
    return render(request, 'gangseo/index.html', {'guzip':guzip})

# /SBZoom/gangseo/eatOut
def eatOut(request):
    q=GuModel.objects.filter(Q(code="E")) # gangseo_gumodel 에서 code가 E(외식업)인 데이타
    ...
    title=['강서구 외식업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
     
    ana_zip = list(zip(qlist,ilban,franchise,total,sales))  

    
    return render(request, 'gangseo/eatOut.html', {'guzip':guzip,'title':title, 'qlist':qlist, 'ilban':ilban,'franchise':franchise,'total':total,'sales':sales,'ana_zip':ana_zip})
    
# /SBZoom/gangseo/service    
def service(request):
    q=GuModel.objects.filter(Q(code="S")) # gangseo_gumodel 에서 code가 S(서비스업)인 데이타
    ...
    title=['강서구 서비스업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'gangseo/service.html',context)

# /SBZoom/gangseo/retail
def retail(request):
    q=GuModel.objects.filter(Q(code="R")) # gangseo_gumodel에서 code가 R(소매업)인 데이타
    ...
    title=['강서구 소매업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'gangseo/retail.html', context)

# /SBZoom/gangseo/total
def total(request):
    ...
    title=['강서구 전체 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'gangseo/total.html', context)
...
def ilban(request):
    ...
    title=['강서구 일반점포 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'gangseo/ilban.html', context)

# /SBZoom/ganseo/franchise
def franchise(request):
    q1=GuModel.objects.filter(Q(code="E")) # gangseo_gumodel에서 code가 E(외식업)인 데이타
    q2=GuModel.objects.filter(Q(code="S")) # gangseo_gumodel에서 code가 S(서비스업)인 데이타
    q3=GuModel.objects.filter(Q(code="R")) # gangseo_gumodel에서 code가 R(소매업)인 데이타   
    q4=GuModel.objects.filter(Q(code="W")) # gangseo_gumodel에서 code가 W(외식+서비스+소매)인 데이타
    ...
    title=['강서구 프렌차이즈 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'gangseo/franchise.html', context)

# /SBZoom/gangseo/totalStores
def totalStores(request):
    ...
    title=['강서구 전체 점포 현황','외식업','서비스업','소매업','전체일반']
    ...
    return render(request, 'gangseo/totalStores.html', context)

# /SBZoom/gangseo/totalSales
def totalSales(request):
    ...
    title=['강서구 전체 점포 현황','외식업 매출액(억)','서비스업 매출액(억)','소매업 매출액(억)','전체 매출액(억)']
    ...
    return render(request, 'gangseo/totalSales.html', context)
