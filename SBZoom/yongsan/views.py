...
# /SBZoom/yongsan/
def index(request):
    return render(request, 'yongsan/index.html', {'guzip':guzip,'menuzip':menuzip})

# /SBZoom/yongsan/eatOut
def eatOut(request):
    q=GuModel.objects.filter(Q(code="E")) # yongsan_gumodel 에서 code가 E(외식업)인 데이타
    ...
    title=['용산구 외식업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
     
    ana_zip = list(zip(qlist,ilban,franchise,total,sales))  

    
    return render(request, 'yongsan/eatOut.html', {'guzip':guzip,'title':title, 'qlist':qlist, 'ilban':ilban,
                                                    'franchise':franchise,'total':total,'sales':sales,'ana_zip':ana_zip,'menuzip':menuzip})
    
# /SBZoom/yongsan/service    
def service(request):
    q=GuModel.objects.filter(Q(code="S")) # yongsan_gumodel 에서 code가 S(서비스업)인 데이타
    ...
    title=['용산구 서비스업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'yongsan/service.html',context)

# /SBZoom/yongsan/retail
def retail(request):
    q=GuModel.objects.filter(Q(code="R")) # yongsan_gumodel에서 code가 R(소매업)인 데이타
    ...
    title=['용산구 소매업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'yongsan/retail.html', context)

# /SBZoom/yongsan/total
def total(request):
    q=GuModel.objects.filter( Q(code="W")) # yongsan_gumodel에서 code가 W(전체)인 데이타
    ...
    title=['용산구 전체 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'yongsan/total.html', context)
    
    
#----------------------------------------------------------------#    
 
# /SBZoom/yongsan/ilban  
def ilban(request):
    q1=GuModel.objects.filter(Q(code="E")) # yongsan_gumodel에서 code가  E(외식업)인 데이타
    q2=GuModel.objects.filter(Q(code="S")) # yongsan_gumodel에서 code가  S(서비스업)인 데이타
    q3=GuModel.objects.filter(Q(code="R")) # yongsan_gumodel에서 code가  R(소매업)인 데이타   
    q4=GuModel.objects.filter(Q(code="W")) # yongsan_gumodel에서 code가  W(외식+서비스+소매)인 데이타
    ...
    title=['용산구 일반점포 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'yongsan/ilban.html', context)

# /SBZoom/yongsan/franchise
def franchise(request):
    ...
    title=['용산구 프렌차이즈 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'yongsan/franchise.html', context)

# /SBZoom/yongsan/totalStores
def totalStores(request):
    ...
    title=['용산구 전체 점포 현황','외식업','서비스업','소매업','전체일반']
    ...
    return render(request, 'yongsan/totalStores.html', context)

# /SBZoom/yongsan/totalSales
def totalSales(request):
    ...
    title=['용산구 전체 점포 현황','외식업 매출액(억)','서비스업 매출액(억)','소매업 매출액(억)','전체 매출액(억)']
    ...
    return render(request, 'yongsan/totalSales.html', context)
