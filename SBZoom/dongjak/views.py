...
# /SBZoom/dongjak/
def index(request):
    return render(request, 'dongjak/index.html', {'guzip':guzip,'menuzip':menuzip})

# /SBZoom/dongjak/eatOut
def eatOut(request):
    q=GuModel.objects.filter(Q(code="E")) # dongjak_gumodel 에서 code가 E(외식업)인 데이타
    ...
    title=['동작구 외식업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
     
    ana_zip = list(zip(qlist,ilban,franchise,total,sales))  

    
    return render(request, 'dongjak/eatOut.html', 
                  {'guzip':guzip,'title':title, 'qlist':qlist, 'ilban':ilban,
                   'franchise':franchise,'total':total,'sales':sales,'ana_zip':ana_zip,'menuzip':menuzip})
    
# /SBZoom/dongjak/service    
def service(request):
    q=GuModel.objects.filter(Q(code="S")) # dongjak_gumodel 에서 code가 S(서비스업)인 데이타
    ...
    title=['동작구 서비스업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'dongjak/service.html',context)

# /SBZoom/dongjak/retail
def retail(request):
    q=GuModel.objects.filter(Q(code="R")) # dongjak_gumodel에서 code가 R(소매업)인 데이타
    ...
    title=['동작구 소매업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'dongjak/retail.html', context)

# /SBZoom/dongjak/total
def total(request):
    q=GuModel.objects.filter( Q(code="W")) # dongjak_gumodel에서 code가 W(전체)인 데이타
    ...
    title=['동작구 전체 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'dongjak/total.html', context)
...
def ilban(request):
    q1=GuModel.objects.filter(Q(code="E")) # dongjak_gumodel에서 code가  E(외식업)인 데이타
    q2=GuModel.objects.filter(Q(code="S")) # dongjak_gumodel에서 code가  S(서비스업)인 데이타
    q3=GuModel.objects.filter(Q(code="R")) # dongjak_gumodel에서 code가  R(소매업)인 데이타   
    q4=GuModel.objects.filter(Q(code="W")) # dongjak_gumodel에서 code가  W(외식+서비스+소매)인 데이타
    ...
    title=['동작구 일반점포 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'dongjak/ilban.html', context)
...
def franchise(request):
    ...
    title=['동작구 프렌차이즈 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'dongjak/franchise.html', context)
...
def totalStores(request):
    ...
    title=['동작구 전체 점포 현황','외식업','서비스업','소매업','전체일반']
    ...
    return render(request, 'dongjak/totalStores.html', context)
...
def totalSales(request):
    ...
    title=['동작구 전체 점포 현황','외식업 매출액(억)','서비스업 매출액(억)','소매업 매출액(억)','전체 매출액(억)']
    ...
    return render(request, 'dongjak/totalSales.html', context)
