...
# /SBZoom/gangse0/
def index(request):
    return render(request, 'geumcheon/index.html', {'guzip':guzip})

# /SBZoom/geumcheon/eatOut
def eatOut(request):
    q=GuModel.objects.filter(Q(code="E")) # geumcheon_gumodel 에서 code가 E(외식업)인 데이타
    ...
    title=['금천구 외식업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
     
    ana_zip = list(zip(qlist,ilban,franchise,total,sales))  

    
    return render(request, 'geumcheon/eatOut.html', {'guzip':guzip,'title':title, 'qlist':qlist, 'ilban':ilban,'franchise':franchise,'total':total,'sales':sales,'ana_zip':ana_zip})
    
# /SBZoom/geumcheon/service    
def service(request):
    q=GuModel.objects.filter(Q(code="S")) # geumcheon_gumodel 에서 code가 S(서비스업)인 데이타
    ...
    title=['금천구 서비스업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'geumcheon/service.html',context)

# /SBZoom/geumcheon/retail
def retail(request):
    q=GuModel.objects.filter(Q(code="R")) # geumcheon_gumodel에서 code가 R(소매업)인 데이타
    ...
    title=['금천구 소매업 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'geumcheon/retail.html', context)

# /SBZoom/geumcheon/total
def total(request):
    ...
    title=['금천구 전체 상권 현황','일반 점포','프렌차이즈','전체','매출액(억)']
    ...
    return render(request, 'geumcheon/total.html', context)
...
def ilban(request):
    ...
    title=['금천구 일반점포 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'geumcheon/ilban.html', context)

# /SBZoom/ganseo/franchise
def franchise(request):
    q1=GuModel.objects.filter(Q(code="E")) # geumcheon_gumodel에서 code가 E(외식업)인 데이타
    q2=GuModel.objects.filter(Q(code="S")) # geumcheon_gumodel에서 code가 S(서비스업)인 데이타
    q3=GuModel.objects.filter(Q(code="R")) # geumcheon_gumodel에서 code가 R(소매업)인 데이타   
    q4=GuModel.objects.filter(Q(code="W")) # geumcheon_gumodel에서 code가 W(외식+서비스+소매)인 데이타
    ...
    title=['금천구 프렌차이즈 현황','외식업','서비스업','소매업','전체일반점포']
    ...
    return render(request, 'geumcheon/franchise.html', context)

# /SBZoom/geumcheon/totalStores
def totalStores(request):
    ...
    title=['금천구 전체 점포 현황','외식업','서비스업','소매업','전체일반']
    ...
    return render(request, 'geumcheon/totalStores.html', context)

# /SBZoom/geumcheon/totalSales
def totalSales(request):
    ...
    title=['금천구 전체 점포 현황','외식업 매출액(억)','서비스업 매출액(억)','소매업 매출액(억)','전체 매출액(억)']
    ...
    return render(request, 'geumcheon/totalSales.html', context)
